...
class _SearchContentPageState extends State<SearchContentPage> {
  ...
  Future<void> _startSearch(String query) async {
    ...
    final cachedIds = await widget.contentCache.listChapterIds(widget.bookId);
    if (!mounted || _cancelled || generation != _searchGeneration) return;

    for (var i = 0; i < widget.chapters.length; i++) {
      if (_cancelled || !mounted || generation != _searchGeneration) break;
      ...
      if (i == widget.durChapterIndex) {
        ...
        final san = widget.contentCache.sanitizeChapterId(ch.id);
        if (cachedIds.contains(san)) {
          content = await widget.contentCache.get(widget.bookId, ch.id);
      ...
      if (!mounted || _cancelled || generation != _searchGeneration) break;
      setState(() {
