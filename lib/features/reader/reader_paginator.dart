...
import 'reader_long_url_breaks.dart';
...
abstract final class ReaderPaginator {
  ...
  static List<ReaderPageSlice> paginate({
    ...
    final effectiveStyle = _styleWithRenderedLineHeight(
      style,
      renderedLineHeight,
    );
  ...
  static List<ReaderPageSlice> _paginateSingleImageStyle({
    ...
    required double maxWidth,
    required double maxHeight,
    required double? renderedLineHeight,
  ...
  static TextStyle _styleWithRenderedLineHeight(
    TextStyle style,
    double? renderedLineHeight,
  ) {
    final fontSize = style.fontSize;
    if (renderedLineHeight == null ||
        fontSize == null ||
    ...
    return style.copyWith(height: renderedLineHeight / fontSize);
  }
  ...
  static void _appendSegment({
    ...
    for (final page in _paginateText(
      ...
      if (renderedLineHeight != null && page.end == segment.length) {
        if (segment.endsWith('\n')) {
          pageEnd--;
        } else if (end == text.length && !pageText.endsWith('\n')) {
          pageText += '\n';
        }
      }
      pages.add(
  ...
  static List<ReaderPageSlice> _paginateText({
    ...
    final longUrlLayout = ReaderLongUrlBreaks.prepare(
      text,
      protectedRanges: [
        for (final placeholder in measuredPlaceholders)
          ReaderLongUrlProtectedRange(placeholder.start, placeholder.end),
      ],
    );
    final layoutText = longUrlLayout.layoutText;
    final layoutPlaceholders = [
      for (final placeholder in measuredPlaceholders)
        ReaderPaginatorPlaceholder(
          start: longUrlLayout.layoutOffsetForSourceBoundary(placeholder.start),
          end: longUrlLayout.layoutOffsetForSourceBoundary(
            placeholder.end,
            end: true,
          ),
          width: placeholder.width,
    ...
    final painter = TextPainter(
      text: _textSpanWithPlaceholders(layoutText, style, layoutPlaceholders),
      textDirection: TextDirection.ltr,
      textAlign: textAlign,
    );
    if (layoutPlaceholders.isNotEmpty) {
      painter.setPlaceholderDimensions([
        for (final placeholder in layoutPlaceholders)
    ...
    for (final metric in metrics) {
      ...
      final layoutRange = painter.getLineBoundary(position);
      if (layoutRange.isValid) {
        final range = longUrlLayout.sourceRangeForLayoutRange(
          layoutRange.start,
          layoutRange.end,
        );
        ...
        if (lines.isEmpty ||
          ...
          lines.add((
            ...
            bottom: _lineBottom(metric, paragraphOffset, renderedLineHeight),
          ));
    ...
    while (start < text.length) {
      ...
      for (var i = lineIndex; i < lines.length; i++) {
        final line = lines[i];
        if (i > lineIndex) {
          final usesFixedLineSteps =
              renderedLineHeight != null && measuredPlaceholders.isEmpty;
          if (usesFixedLineSteps) {
            final lineCount = i - lineIndex + 1;
            final maxLineCount = (maxHeight / renderedLineHeight).ceil();
            if (lineCount > maxLineCount) break;
          } else if (line.bottom > pageTop + maxHeight) {
            break;
          }
        }
  ...
  static double _lineBottom(
    LineMetrics metric,
    double paragraphOffset,
    double? renderedLineHeight,
  ) {
    final measuredBottom = metric.baseline + metric.descent + paragraphOffset;
    if (renderedLineHeight == null ||
        !renderedLineHeight.isFinite ||
        renderedLineHeight <= 0) {
      return measuredBottom;
    }
    final lineTop = metric.baseline - metric.ascent + paragraphOffset;
    return math.max(measuredBottom, lineTop + renderedLineHeight);
  }
