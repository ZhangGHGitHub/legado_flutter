...
class _BookInfoPageState extends riverpod.ConsumerState<_BookInfoPageBody> {
    ...
    try {
      ...
      if (foundCover != null && mounted) {
        ...
        if (_isInShelf && foundCover.isNotEmpty) {
          final next = await _metadataPort.updateCover(_book, foundCover);
  ...
  Future<void> _addToShelf() async {
    ...
    await _lifecyclePort.addBook(_book);
    await _lifecyclePort.persistCurrentTocFor(_book);
    if (mounted) {
  ...
  Future<void> _removeFromShelf() async {
    await _lifecyclePort.removeBook(_book.id);
    if (mounted) {
  ...
  Future<void> _editBookInfo() async {
    ...
    if (saved == true && mounted) {
      if (_isInShelf) {
        final next = await _metadataPort.updateBookDetails(
          _book.id,
          name: name,
          author: author,
