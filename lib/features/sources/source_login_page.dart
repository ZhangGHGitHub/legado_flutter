...
class _SourceLoginPageState extends State<SourceLoginPage> {
    ...
    for (final row in _rows) {
      ...
      switch (row.type) {
        ...
        case LoginRowType.select:
          ...
          final initial = chars.contains(def)
              ? def
  ...
  Future<void> _clear() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('已清除登录信息')));
    }
  }
  ...
  Future<void> _showLoginHeader() async {
    ...
    await showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SelectableText((h == null || h.isEmpty) ? '（空）' : h),
        actions: [
  ...
  Future<void> _clearLoginHeader() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('已删除登录头')));
  }
    ...
    try {
      ...
      if (_loginPort.isJsUrl(url)) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('登录脚本完成')));
        }
        return;
      }
  ...
  Future<void> _runButton(LoginRowUi row) async {
    ...
    if (action.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('${row.label}：无动作脚本')));
      return;
    }
    try {
      await _save(snack: false);
      ...
      if (!mounted) return;
      setState(
        () => _status = r.output.isEmpty ? '${row.label} 已执行' : r.output,
      );
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('${row.label} 已执行')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('${row.label} 失败: $e')));
    }
  }
                ...
                if (_jsUi)
                  Card(
                    ...
                    child: const Padding(
                      ...
                      child: Text('本书源 loginUi 为 JS 动态表单，已尝试执行并渲染返回的字段。'),
                    ),
                  ),
                if (_rows.isEmpty)
                  Padding(
