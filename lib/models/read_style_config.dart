...
/// 阅读排版/主题色配置（对齐 legado `ReadBookConfig.Config` 子集）
class ReadStyleConfig {
  ...
  Color? get dayBgColor => bgType == 0 ? parseColor(bgStr) : null;
  Color? get nightBgColor => bgTypeNight == 0 ? parseColor(bgStrNight) : null;
  Color? get dayTextColor => parseColor(textColor);
  Color? get nightTextColor => parseColor(textColorNight);
  Color? get dayAccentColor => parseColor(textAccentColor);
  Color? get nightAccentColor => parseColor(textAccentColorNight);
}

/// 阅读主题槽位的本地覆盖（实色；图片路径另存）
class ReadStyleSlotOverride {
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (background != null) 'background': ReadStyleConfig.toHex(background!),
        if (text != null) 'text': ReadStyleConfig.toHex(text!),
        if (accent != null) 'accent': ReadStyleConfig.toHex(accent!),
  ...
  factory ReadStyleSlotOverride.fromJson(Map<String, dynamic> json) {
    return ReadStyleSlotOverride(
      name: json['name']?.toString(),
      background: ReadStyleConfig.parseColor(json['background']?.toString()),
      text: ReadStyleConfig.parseColor(json['text']?.toString()),
      accent: ReadStyleConfig.parseColor(json['accent']?.toString()),
