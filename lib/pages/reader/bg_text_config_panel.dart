...
class _BgTextConfigSheetState extends State<_BgTextConfigSheet> {
  ...
  void _applyImported(ReadStyleConfig config) {
    ...
    setState(() {
      ...
      _typographyFromZip = widget.settings.copyWith(
        ...
        paddingHorizontal:
            ((config.paddingLeft + config.paddingRight) / 2).toDouble(),
        paddingVertical:
            ((config.paddingTop + config.paddingBottom) / 2).toDouble(),
  ...
  Future<void> _exportZip() async {
    ...
    try {
      ...
      final safe =
          (_name.isEmpty ? 'readConfig' : _name).replaceAll(RegExp(r'[\\/:*?"<>|]'), '_');
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              saved == null ? '已取消导出' : '导出成功, 文件名为 $fileName',
            ),
          ),
        );
      }
