...
class _SourcesPageState extends State<SourcesPage> {
  ...
  List<BookSource> _visibleSources(List<BookSource> all) {
    ...
    int cmp(BookSource a, BookSource b) {
      final r = switch (_sort) {
        ...
        _SourceSort.enabled =>
          (b.enabled ? 1 : 0).compareTo(a.enabled ? 1 : 0),
        _SourceSort.lastUpdate =>
          a.lastUpdateTime.compareTo(b.lastUpdateTime),
  ...
  Future<void> _batchEnable(bool enabled) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(enabled ? '已启用 $n 个书源' : '已禁用 $n 个书源'),
      ),
    );
  }
              ...
              return PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  ...
                  PopupMenuItem(
                    value: 'explore_on',
                    child:
                        Text('启用发现', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'explore_off',
                    child:
                        Text('禁用发现', style: _uiText(color: scheme.onSurface)),
                  ),
        ...
        child: Row(
          children: [
            ...
            Switch(
              ...
              onChanged: (v) =>
                  provider.toggleSource(s.bookSourceUrl, v),
            ),
            IconButton(
              tooltip: '编辑',
  ...
  Widget _buildSelectActionBar(ColorScheme scheme, List<BookSource> visible) {
    ...
    final selectedCount =
        visible.where((s) => _selected.contains(s.bookSourceUrl)).length;
