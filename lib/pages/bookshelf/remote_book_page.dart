...
class RemoteBookPage extends StatefulWidget {
  const RemoteBookPage({super.key});
...
class _RemoteBookPageState extends State<RemoteBookPage> {
  ...
  static final _archiveExt = RegExp(
    r'\.(zip|rar|7z)$',
    caseSensitive: false,
  );
  ...
  Future<void> _reload() async {
    ...
    try {
      // TODO: 对齐 AppWebDav — 首次进入时 MKCOL 确保 books/ 存在（Rust 暂无 mkdir）。
      final list = await webdavList(
        url: cfg.url,
      ...
      if (!mounted) return;
      final raw = '$e';
      // TODO: 对齐 RemoteBookWebDav.init — MKCOL 自动创建 books/（Rust 暂无 mkdir）。
      final hint = raw.contains(RegExp(r'404|Not Found|PROPFIND', caseSensitive: false))
          ? '\n\n若尚未创建远程「${cfg.booksDir}」目录，请在 WebDAV 上手动建好后再刷新。'
          : '';
      setState(() {
        _loading = false;
        _error = '获取webDav书籍出错\n$raw$hint';
  ...
  List<WebDavEntry> get _visibleEntries {
    ...
    int cmpName(WebDavEntry a, WebDavEntry b) =>
        a.name.toLowerCase().compareTo(b.name.toLowerCase());
    // TODO: WebDavEntry 尚无 lastModify；时间排序暂用名称（与 Jingshiro Default 不完全一致）。
    int cmpTime(WebDavEntry a, WebDavEntry b) => cmpName(a, b);

    list.sort((a, b) {
      if (a.isDir != b.isDir) return a.isDir ? -1 : 1;
      final c = _sort == _RemoteSort.name ? cmpName(a, b) : cmpTime(a, b);
  ...
  void _toggleSelect(WebDavEntry e) {
    if (e.isDir || !_isImportable(e.name)) return;
    setState(() {
  ...
  void _selectAllVisible(bool all) {
    final files = _visibleEntries
        .where((e) => !e.isDir && _isImportable(e.name))
        .toList();
    setState(() {
      if (all) {
  ...
  Future<void> _addSelectedToShelf() async {
    ...
    final targets =
        _entries.where((e) => _selected.contains(e.path)).toList(growable: false);
    ...
    try {
      ...
      for (final e in targets) {
        if (!_isImportable(e.name)) {
          // TODO: 压缩包 / umd|pdf|mobi 等 — 对齐 LocalBook.importFiles
          fail++;
          await AppLog.e('导入出错\n暂不支持: ${e.name}');
          continue;
        }
        try {
          final bytes = await webdavDownload(
            url: cfg.url,
            username: cfg.account,
            password: cfg.password,
            remotePath: e.path,
  ...
  Widget build(BuildContext context) {
    ...
    final checkable =
        visible.where((e) => !e.isDir && _isImportable(e.name)).length;
              ...
              onSelected: (v) async {
                if (v == 'server') {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('WebDAV 远程书籍帮助页尚未移植'),
                    ),
                  );
                }
              },
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        TextButton(
                          onPressed:
                              _importing ? null : _invertVisibleSelection,
                          child: const Text('反选'),
                        ),
                        Expanded(
                          child: Text(
      ...
      builder: (ctx, books, _) {
        return ListView.separated(
          ...
          itemBuilder: (ctx, i) {
            ...
            final selectable = !e.isDir && _isImportable(e.name);
            ...
            return ListTile(
              ...
              onTap: () {
                if (e.isDir) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('压缩包导入尚未支持（需解压后端）'),
                    ),
                  );
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('暂不支持导入: ${e.name}'),
                    ),
                  );
                }
              },
