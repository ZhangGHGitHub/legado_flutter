...
class _ThemeConfigPageState extends State<ThemeConfigPage> {
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ...
            OutlinedButton.icon(
              onPressed: _loadingMarket ? null : () => _showMarketDialog(themeCtrl),
  ...
  void _exportTheme(ThemeModeController ctrl) {
    final json = const JsonEncoder.withIndent('  ').convert(ctrl.exportConfig());
  ...
  Future<void> _showMarketDialog(ThemeModeController ctrl) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('主题已从 URL 加载')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _applyJson(ThemeModeController ctrl, String raw) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('主题已导入')),
        );
      }
    } catch (e) {
      if (mounted) {
