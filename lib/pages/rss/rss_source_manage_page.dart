...
class _RssSourceManagePageState extends State<RssSourceManagePage> {
  ...
  List<RssSource> _visible(RssProvider p) => p.managedSources(
    ...
    switch (value) {
      case 'manage':
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('「分组管理」暂未实现')),
        );
      case 'enabled':
        _setFilter('enabled');
      case 'disabled':
  ...
  Future<void> _onOverflow(String value) async {
    switch (value) {
      ...
      case 'import_default':
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('「导入默认规则」暂未实现')),
        );
      case 'help':
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _importLocal() async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(ok ? '导入成功' : '导入失败，请检查文件内容')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _importOnline() async {
    ...
    final success =
        await context.read<RssProvider>().importSourcesFromUrl(url);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(success ? '导入成功' : '导入失败，请检查 URL')),
    );
  }
  ...
  Future<void> _importQr() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(success ? '导入成功' : '导入失败')),
    );
  }
  ...
  Future<void> _importPaste() async {
    ...
    if (ok != true || !mounted) return;
    final success =
        await context.read<RssProvider>().importSources(controller.text);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _onBottomMore(String value) async {
    ...
    if (_selected.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('请先选择订阅源')),
      );
      return;
    }
    switch (value) {
      ...
      case 'export':
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('已复制选中源到剪贴板')),
        );
      default:
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('该批量操作暂未实现')),
        );
    }
  }
              ...
              return PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  PopupMenuItem(
                    value: 'manage',
                    child:
                        Text('分组管理', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'enabled',
                    child:
                        Text('已启用', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'disabled',
                    child:
                        Text('已禁用', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'login',
                    child:
                        Text('需要登录', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'null_group',
                    child:
                        Text('未分组', style: _uiText(color: scheme.onSurface)),
                  ),
                          ...
                          children: [
                            ...
                            Builder(
                              builder: (tileCtx) => IconButton(
                                ...
                                onPressed: () {
                                  final box = tileCtx.findRenderObject()
                                      as RenderBox?;
  ...
  Widget _buildSelectActionBar(ColorScheme scheme, List<RssSource> visible) {
    ...
    final selectedCount =
        visible.where((s) => _selected.contains(s.sourceUrl)).length;
              ...
              LegadoBottomBarPopupButton<String>(
                ...
                itemBuilder: (_) => [
                  PopupMenuItem(
                    value: 'enable',
                    child:
                        Text('启用所选', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'disable',
                    child:
                        Text('禁用所选', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'top',
                    child:
                        Text('置顶所选', style: _uiText(color: scheme.onSurface)),
                  ),
                  PopupMenuItem(
                    value: 'export',
                    child:
                        Text('导出所选', style: _uiText(color: scheme.onSurface)),
                  ),
                ],
              ),
            ],
