...
async fn bishu_ch0_and_source_shape() {
    ...
    println!("is_json_api={} rule_content_len={} starts_js={}",
        bs.is_json_api(), bs.rule_content.len(), bs.rule_content.trim_start().starts_with("<js>"));
    let url = "http://m.biqukun.org/44/44671/".to_string();
    let info = get_book_info(source.clone(), url.clone()).await.unwrap();
    let toc = if info.toc_url.is_empty() { url } else { info.toc_url };
    let ch = get_toc(source.clone(), toc).await.unwrap();
    println!("n={} url0={}", ch.len(), ch[0].url);
    let c = get_content(source.clone(), ch[0].url.clone()).await.unwrap();
    println!("len={} ph? {} head={:?}", c.len(), c=="（此章节暂无内容）", c.chars().take(30).collect::<String>());
    // also try chapter near 800 if exists
    if ch.len() > 800 {
        let c2 = get_content(source.clone(), ch[800].url.clone()).await.unwrap();
