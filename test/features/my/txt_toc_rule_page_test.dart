...
import 'package:legado_flutter/application/preferences/txt_toc_rule_prefs_port.dart';
...
void main() {
  ...
  testWidgets('copies a TXT toc rule through the shared clipboard port', (
    ...
    final preferences = _FakeTxtTocRulePrefs([rule]);

    await tester.pumpWidget(
      MultiProvider(
        providers: [
          Provider<ClipboardPort>.value(value: clipboard),
          Provider<TxtTocRulePrefsPort>.value(value: preferences),
        ],
...
class _FakeTxtTocRulePrefs implements TxtTocRulePrefsPort {
  _FakeTxtTocRulePrefs(this.rules);

  List<TxtTocRule> rules;

  @override
  List<TxtTocRule> get defaultRules => const [];

  @override
  Future<List<TxtTocRule>> load() async => List.of(rules);

  @override
  Future<void> save(List<TxtTocRule> rules) async {
