...
void main() {
  testWidgets('换源页通过共享 SourceController 搜索并完成选源', (tester) async {
    ...
    final original = Book(
      ...
      name: '换源书',
      author: '作者',
      sourceUrl: 'https://old.example/book',
    );
    ...
    expect(find.text('换源书'), findsOneWidget);

    await tester.tap(find.text('换源书'));
    await tester.pumpAndSettle();

    expect(find.text('已选择：https://source.example/book'), findsOneWidget);
  });
}

class _ChangeSourceHarness extends StatefulWidget {
  const _ChangeSourceHarness({required this.book});

  final Book book;

  @override
  State<_ChangeSourceHarness> createState() => _ChangeSourceHarnessState();
}

class _ChangeSourceHarnessState extends State<_ChangeSourceHarness> {
  Book? selected;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: TextButton(
        ...
        child: Text(selected == null ? '打开换源' : '已选择：${selected!.sourceUrl}'),
      ),
    );
  }
}
...
final class _MemorySourceRepository implements BookSourceRepository {
  ...
  @override
  Future<void> upsert(BookSource source) async => update(source);
...
final class _TocPort implements BookSourceTocPort {
  const _TocPort();
