...
> 2026-08-17 Phase 4/R6 封面换源与数据库迁移基线：详情页通过 application `ChangeCoverController`、候选缓存和停止/继续控制连接 Rust 规则端口，书源封面 `coverUrl` 与用户封面 `customCoverUrl` 独立存储；Rust schema v18 保留 Room v99 archive-only 导入，旧 JSON 缺省自定义封面字段时保持已有值。默认 `coverRule.json` 与原版逐字节一致，SHA-256 为 `94C21F05E52528EB7AF15B9FC26EAC29911E3D020BDB064946FBA68868D1DD2E`。QuickJS 兼容层提供全局 `com`、静态 `JsonPath.read`，`java.ajaxAll` 由 Rust 独立线程并发执行并按输入顺序回传。迁移测试、Rust 核心 `277` 项、Flutter 封面定向 `36/36`、FRB/集成 `59` 项（`3` 跳过）和 Flutter 全量 `1326` 项（`3` 跳过）通过；`flutter analyze --no-pub`、架构边界、格式和 `git diff --check` 通过。在线封面规则真实外网验收仍缺失，R6 尚未退出；正文、目录顺序、分页、章节身份、UTF-16 阅读位置、第 3 条断行规则、R1-12 和暂停平台边界不变。

> 2026-08-07 Phase 4/R6 RSS 订阅源帮助：原版 `RssSourceActivity` 的帮助入口固定为 `SourceMRssHelp`，资产仅含标题与一、二级无序列表，不需要新增网络、外链或状态持久化能力。应用将原版资产逐字节注册，`RssSourceManagePage` 仅调用共享 UI 帮助组件；该组件的可配置标题/资产路径仍在 widget 边界，未引入 Feature 到 infrastructure 的依赖。嵌套列表解析以任意前导空白识别原版 `* ` 项，书源帮助保持兼容。SHA-256 对照、RSS/书源帮助定向 `7/7`、Flutter 全量 `1305`（`3` 项既有条件跳过）、`flutter analyze --no-pub`、架构边界和 `git diff --check` 通过；远程书籍 WebDAV 帮助仍因完整 Markdown/外链/版本状态契约保持门禁，R6 尚未退出。
...
> 2026-08-05 Phase 4/R6 Reader 模拟追读、当前目录与 BookInfo 元数据边界：`ReaderPage` 的模拟追读和当前目录读取通过显式 application port、共享 port 或明确空实现获取，页面不再直接用 `BookProvider` 组装这两个 fallback；`BookInfoPage` 元数据写入通过 `BookMetadataPort`，生产适配器继续复用 `BookMetadataController` 的字段裁剪和 Provider 快照语义。定向联合 `28/28`、Flutter 全量 `1289`（`3` 项既有条件跳过）、Rust workspace `270` 测试、`flutter analyze --no-pub`、架构边界和 `git diff --check` 通过；正文、目录顺序、分页、章节身份、UTF-16 阅读位置和第 3 条断行规则保持不变，R6 尚未退出。

> 2026-08-05 Phase 4/R6 Reader 与 BookInfo 端口 fallback 边界：`ReaderPage` 的进度写入和章节缓存状态通过显式 application port 或组合根真实 port 获取；`BookInfoPage` 的目录加载通过显式 `BookInfoChapterPort`、共享 Provider port 或明确空实现获取。Android 快照宿主显式保留原 `BookProvider` 回调，未创建第二份事实源；正文、目录顺序、分页、章节身份、UTF-16 阅读位置和第 3 条断行规则保持不变。定向联合 `26/26`、Flutter 全量 `1288`（`3` 项既有条件跳过）、`flutter analyze --no-pub`、架构边界和 `git diff --check` 通过；R6 尚未退出。
