...
2026-08-17 R6 封面换源与 schema v18 批次追溯：先通过 Rust Room v99 映射、数据库迁移和 QuickJS 定向门禁，再执行 Rust 全量、Flutter 封面定向、FRB 引擎/集成定向和 Flutter 全量；测试失败时先重建与 Dart 绑定匹配的 `rust/target/release/legado_engine.dll`，未放宽断言或跳过失败。实现记录了 `coverUrl/customCoverUrl` 独立语义、旧 JSON merge 不清空自定义封面、默认 `coverRule.json` 原版逐字节复制及哈希 `94C21F05E52528EB7AF15B9FC26EAC29911E3D020BDB064946FBA68868D1DD2E`、QuickJS `com/JsonPath.read`、`ajaxAll` 并发宿主桥、application controller/缓存/停止继续和详情入口。实际结果：迁移定向 `1/1`、数据库定向 `33/33`、Rust 核心全量 `277` 项通过、Flutter 封面定向 `36/36`、FRB 引擎/集成 `59` 项且 `3` 项条件跳过、Flutter 全量 `1326` 项且 `3` 项条件跳过；`flutter analyze --no-pub` 无问题，架构边界通过，格式和 `git diff --check` 通过。在线封面规则没有真实外网验收证据，R6 不标记退出；继续保留 Room v99 archive-only、R1-12、原版 UI 对照和暂停平台门禁。只提交本批文件，使用中文 commit，不自动 push。

2026-08-07 Phase 4/R6 RSS 订阅源管理帮助追溯：只读核对原版 `RssSourceActivity` 的 `showHelp("SourceMRssHelp")` 调用和 `SourceMRssHelp.md` 资产，确认该 Markdown 仅使用标题及一、二级无序列表。将资产逐字节复制并注册，RSS 页面复用既有帮助组件；组件改为接受资产路径和标题，且将任意缩进的 `* ` 行渲染为二级列表，保持已存在书源帮助兼容。测试固定点击“帮助”后显示 RSS 标题、一级说明和二级操作项，并复验书源管理页。原版资产 SHA-256 对照、定向 Flutter `7/7`、Flutter 串行全量 `1305`（`3` 项既有条件跳过）、`flutter analyze --no-pub`、`scripts/check_architecture_boundaries.ps1`、Dart 格式和 `git diff --check` 均通过；只提交本批 RSS 页面、共享帮助组件、资产、测试和追溯文档，不提交用户本地文件，不自动 push。
