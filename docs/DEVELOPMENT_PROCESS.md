...
R1-12 状态更正（2026-08-03）：archive-only 产品边界已完成，历史记录中“R1-12 仍不退出”仅表示当时尚未确认产品边界；当前仍未关闭的只是 `readRecord` 统计语义、非核心表 Rust v17 业务化和真实原版非空数据库证据。真实证据补齐前不推进新的 R2-R6 生产实现。
