...
状态：R0 基线退出就绪，等待确认进入 R1。日期：2026-07-27。本文是当前 Rust + Flutter 重构的 R0 工作记录，
...
| 阶段 | 当前判定 | 需要的后续证据 |
|---|---|---|
| R0 | 重新打开 | 本文、工件分类、静态边界检查和当前测试记录 |
| R1 | 重新打开，R1-12 进行中 | Kotlin Room v99 探针、字段映射、导入、备份/回滚和真实 fixture |
| R2 | 部分完成，待迁移残留网络 | Rust 网络调用链、TLS、订阅和书单契约 |
| R3 | 部分完成，待迁移正文/替换 | Dart/Rust 双跑、正文和分页快照 |
| R4 | 历史门禁有记录 | 仅在目录行为回归时重新开启 2A/2B |
| R5 | 开发门禁有记录，未架构完成 | 外部 WebDAV 验收和 Flutter 本地 Web 服务 |
| R6 | 未退出 | Feature 收敛、受控 UI 验收、平台与发布门禁 |
