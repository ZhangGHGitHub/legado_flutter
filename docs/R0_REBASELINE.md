...
| R1 | Room v99 门禁和组装边界已完成 | 非核心 archive-only 表的独立产品语义 |
| R2 | 书源、网络、Cookie 与可见 WebView 宿主门禁完成 | 未命中的第三方 JS 宿主兼容 backlog |
| R3 | 正文、缓存与远端 ZIP 门禁完成 | 继续保护断行、分页和 UTF-16 位置契约 |
| R4 | 历史门禁有记录 | 仅在目录行为回归时重新开启 2A/2B |
| R5 | 本地开发门禁及本地 Web 服务归属迁移完成，发布验收未完成 | 正式或主流 WebDAV 服务真实验收 |
| R6 | 功能域迁移和 P0-1 崩溃恢复完成，未退出 | P0-2 存储初始化安全、应用用例依赖、受控 UI 与平台门禁 |
