...
当前进度：R6 功能域目录迁移已完成 `main`、`bookshelf`、`reader`、`book`、`sources`、`rss`、`settings`、`my`、`search`、`cache`、`code_edit`、`explore`、`ai`、`obsidian` 和 `common`；旧 `lib/pages` 下仅保留书架兼容导出。P0-1 崩溃防护与启动恢复已完成，Flutter 全量 `659` 通过（`3` 项既有条件跳过）、Rust workspace 核心 `184/184`、analyze、Android/Windows debug 构建均通过。R6 下一固定任务是 P0-2 存储初始化安全；其后仍有应用用例依赖、受控 UI/目标平台和发布门禁。真实 Android TTS、后台音频服务、Web/WASM/PWA、外部 AI 服务及正式/主流 WebDAV 验收继续按暂停/范围外条件处理。
...
P0-1 当前证据：纯 Dart 报告/存储 port、SharedPreferences adapter、`FlutterError`/`PlatformDispatcher`/Zone 捕获、启动阶段记录、真实应用/平台/引擎版本和原版顺序的一次性提示均已接入；错误与堆栈有 UTF-16 安全长度上限，存储/元数据失败不阻塞启动。崩溃链定向 `13/13`、启动相关回归 `15/15`、Flutter/Rust 全量与双平台构建通过。下一项严格进入 P0-2，不在本批顺带迁移 `MainShell` 隐私偏好或 `AppLog`。
...
当前已完成 **R0 架构盘点与行为基线**、**R1（含 Room v99 迁移）**、**R2 书源/网络边界**、**R3 正文/缓存/远端 ZIP**、**R4 目录复核** 和 **R5 本地开发门禁/本地 Web API 归属迁移**。R6 功能域迁移、analyze 与 Android/Windows 构建已有历史证据，横切基础设施 P0-1 崩溃防护与启动恢复现已通过；下一步按固定顺序推进 P0-2 存储初始化安全。架构扫描仍有既有 `146` 条 Feature 偏好/业务 service backlog，不能当作例外或最终退出。发布前正式/主流 WebDAV、Web/WASM/PWA 与真实 Android TTS 继续暂停。逐项记录见 [`REFACTOR_ARCHITECTURE_BASELINE.md`](./REFACTOR_ARCHITECTURE_BASELINE.md)，不得改变正文、目录、分页、章节身份、UTF-16 位置和第 3 条断行规则。
