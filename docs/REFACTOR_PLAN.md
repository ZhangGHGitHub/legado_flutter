...
2026-08-17 Phase 4/R6 封面换源与数据库迁移兼容收口：封面换源通过 application controller、缓存端口、停止/继续搜索和详情页入口接入，独立维护 `coverUrl`（书源封面）与 `customCoverUrl`（用户封面）；Rust 数据库 schema 升至 v18，保留 Room v99 archive-only 导入边界，旧 JSON 缺少 `customCoverUrl` 时不清空已有自定义封面。默认 `coverRule.json` 按原版逐字节复制，应用与原版 SHA-256 均为 `94C21F05E52528EB7AF15B9FC26EAC29911E3D020BDB064946FBA68868D1DD2E`。QuickJS 补齐全局 `com`、静态 `JsonPath.read` 和 `ajaxAll` 响应契约，Rust 宿主按输入顺序并发请求。Room v99 探针/映射继续保留原始字段和未映射登记，不改变正文、目录顺序、分页、章节身份、UTF-16 阅读位置、第 3 条断行规则或 R1-12。实际验证：Rust 核心 `277` 项通过，Flutter 封面定向 `36/36`、FRB 引擎/集成定向 `59` 项（`3` 项条件跳过）、Flutter 全量 `1326` 项（`3` 项条件跳过），`flutter analyze --no-pub`、架构边界、Dart/Rust 格式和 `git diff --check` 通过。在线封面规则尚未做真实外网验收；R6 尚未退出，Web/WASM/PWA、真实 Android TTS 和正式 WebDAV 继续暂停。
...
2026-08-06 Phase 4/R6 Reader 残余 Provider 清理与 RSS 能力门禁：`ReaderPage` 移除最后一个无业务用途的 `BookProvider` 依赖及 `didChangeDependencies` 缓存逻辑，`_saveProgress` 继续通过既有 application 端口写入；`RssSourceManagePage` 对分组管理、默认规则和帮助保留明确占位，并登记缺失的 application/资源契约；书架菜单并行审查确认导入/导出等动作已有统一分发和组合根端口注册，不重复实现。合并定向测试 `13/13`；Flutter 全量 `1297`（`3` 项既有条件跳过）、`flutter analyze --no-pub`、架构边界和 `git diff --check` 全部通过；本批未修改 Rust、`legado-main/`、正文、目录顺序、分页、章节身份、UTF-16 阅读位置、第 3 条断行规则、R1-12 或暂停平台门禁，R6 尚未退出。
