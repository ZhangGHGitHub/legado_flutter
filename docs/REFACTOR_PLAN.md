...
本附录以当前工作树为依据，重新打开 R0 的“可追溯基础”门禁。此前 R0-R6 的记录保留为历史
迁移证据，不能在当前约三百项未提交变更尚未拆分、且 Dart 仍保留平台无关业务逻辑时，直接视为
最终架构验收。
...
- R0 退出前不得把 R1-R6 标记为“全面完成”；R1-R4 的既有记录需要按当前代码和针对性测试复验，
  R5 的正式/主流 WebDAV 发布前验收、R5 本地 Web 服务归属修正和 R6 最终收敛仍未完成。
...
##### R1-12：Kotlin Room v99 数据迁移门禁（当前）
