...
2026-08-03 Phase 4/R6 书架同步前置批次：三个 agent 完成书架两种样式、入口和读写同步风险审查；owner 增加 `BookProvider.loadBooks` 请求失效保护及 application `BookshelfChangePort`/`BookshelfChangeBus`。生产组合根共享变更总线，Provider 书架写入成功后发布 revision，`BookshelfNotifier` 通过既有 `refresh()` 读取最新快照；失败不发布。定向 `16/16`、Flutter 全量 `1151`（`3` 项既有条件跳过）、`flutter analyze --no-pub`、架构边界/脚本自测和 `git diff --check` 通过。书架页面仍以 `BookProvider` 为展示源，下一步才评估单一初始化来源和只读 UI 切换；本批不改变 Reader、正文、目录、分页、章节身份、UTF-16 阅读位置、R1-12 或暂停平台门禁，R6 尚未全量退出。
