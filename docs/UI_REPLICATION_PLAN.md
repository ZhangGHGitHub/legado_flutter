...
| 更多菜单 | 新建/本地/网络/二维码/按域名分组/帮助 | ✅ 对齐 Jingshiro（**不含**粘贴/市场/校验——管理页溢出无此三项） | 帮助占位 |
...
| 导入方式 | JSON/URL/二维码/剪贴板 | ✅ Wave 2：解析后预览勾选再导入 | 网络导入 URL 历史（Wave 3） |
| 校验 | 关键词 + 设置 + 状态点 | ✅ Wave 2：`CheckSourceKeywordDialog` + `CheckSourceConfigDialog` + 持久化 | 行内 debug 文案（Wave 3） |
...
#### Task UI-4: 书源管理对齐 (`sources_page.dart`) — 🟡 Wave 2 完成（2026-07-21）；Wave 3 仍开放
...
- [ ] **Wave 3**：帮助页、滑动多选、网络导入 URL 历史、行内校验 debug 文案
...
Task UI-4:  书源管理 Wave 2（导入预览/校验/多分组）  🟡 Wave 2 完成（2026-07-21）；Wave 3 仍开放
...
| 书源管理 | 98% | UI-4 Wave 2（2026-07-21）：导入预览/校验关键词+设置/逗号多分组/校验持久化；Wave 3（帮助/滑动多选/URL 历史/行内 debug）仍开放 |
