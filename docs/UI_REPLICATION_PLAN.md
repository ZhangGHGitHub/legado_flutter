...
- [x] 信息区基础版：时间、页码、全书进度%（电量：`battery_plus` 真值）
...
| "读完" / "N刷" 标签 | `read_badge` | ✅ UI-6 `ReadBadge`（读完/N刷） | 详情可标记；书架同步展示 |
...
|--------|-----------|-------------|------|
| 封面 + 书名/作者/书源 | ✅ | ✅ | 需核对布局 |
| 简介 | 可展开/收起 | ✅ UI-6 展开/收起 | |
| 操作按钮 | 加入书架 / 阅读 / 换源 | ✅ UI-6 三钮主入口 | |
| 缓存进度 | 进度条 + x/y 已缓存 | ✅ | |
| 读完/N刷 标签 | `readIteration` | ✅ UI-6 `ReadBadge`+详情标记 | schema v10 |
| 章节目录 | 嵌入或跳转 | ✅ | |
...
#### Task UI-6: 书籍详情补全 (`book_info_page.dart`) — ✅ 基本完成（2026-07-14）

- [x] 「换源」按钮明显入口（主操作三钮：书架 / 阅读 / 换源）
- [x] 简介可展开/收起
- [x] 读完/N刷标签（`Book.readIteration` + DB schema v10 + `ReadBadge`）
