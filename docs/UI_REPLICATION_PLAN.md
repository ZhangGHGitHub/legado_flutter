...
- [ ] 书名（完整）
- [ ] 换源、刷新、离线缓存
- [ ] 更多：书签、拷贝内容、翻页动画(本书)、拉取云端进度、反转内容、替换净化开关、重新分段、更新目录、书籍信息
- [ ] 章节 + 书源地址（两行）
- [ ] 顶栏自动隐藏
...
| 2 | `activity_book_read.xml` | 阅读器 | ✅ `reader_page.dart` | **缺底栏章进度/页码/时间、顶栏自动隐藏、书票 overlay** |
...
| 对照项 | Jingshiro | Flutter 现状 | 差距 |
|--------|-----------|-------------|------|
| **顶栏** | 自动隐藏；返回/书名/更多(目录/设置/AI/书票) | 有 AppBar | **缺自动隐藏、书名居中、更多菜单项完整度** |
| **正文** | 左右翻页/滚动/仿真 | ✅ | 需核对仿真翻页手势 |
| **阅读主题** | 米黄/白/暗/绿 + 自定义 | ✅ | 需核对预设值 |
| **底栏** | 章节进度条 + 页码 + 时间(可选) | ❌ **完全缺失** | **核心 UI 差距** |
| **设置面板** | 字号/行距/翻页/主题/字体/边距/TTS/更多 | ⚠️ 有基本版 | **缺字体/边距/TTS/更多** |
| **TTS 朗读** | `dialog_read_aloud.xml` | ❌ | |
| **自动阅读** | `dialog_auto_read.xml` 定时翻页 | ❌ | |
| **正文搜索** | `activity_search_content.xml` | ❌ | |
| **目录浮层** | BottomSheet + 当前章高亮 | ✅ `toc_sheet.dart` | 需核对 |
| **书签** | 点击书签按钮保存 | ⚠️ 书签页存在 | **阅读器内缺书签按钮** |
...
| **点击行为配置** | `dialog_click_action_config.xml` | ❌ | |
| **翻页按键配置** | `dialog_page_key.xml` | ❌ | |
| **AI 入口** | 侧滑/FAB → AiChat | ✅ `ai_chat_page.dart` | 需核对入口位置 |
| **亮度** | 跟随系统/手动 | ❓ 待确认 | |
| **屏幕方向** | 锁定/自由 | ❌ | |
| **音量键翻页** | `volKeyTurnPage` | ❌ | |
...
#### Task UI-1: 阅读器底栏 (`reader_page.dart`)
...
- [ ] 底部进度条 + 章节页码 + 时间/电量（可选）
- [ ] 顶栏自动隐藏（点击切换）
- [ ] 更多菜单完整项（目录/设置/AI/书票/换源/书签）

#### Task UI-2: 阅读器设置补全 (`reader_settings.dart`)

对齐 `dialog_read_bg_text.xml` + `dialog_read_book_style.xml`：

- [ ] **字体选择器**（`dialog_font_select.xml`）— 内置字体列表 + 自定义导入
- [ ] **边距设置**（`dialog_read_padding.xml`）— 上/下/左/右独立调节
- [ ] **TTS 朗读设置**（`dialog_read_aloud.xml`）— 语速/音调/引擎
- [ ] **自动阅读**（`dialog_auto_read.xml`）— 定时翻页速度
- [ ] **点击行为配置**（`dialog_click_action_config.xml`）— 上/中/下区域行为
- [ ] **翻页按键**（`dialog_page_key.xml`）— 音量键/蓝牙翻页器
- [ ] **更多设置入口** — 屏幕方向/亮度/音量键
...
#### Task UI-7: 书架长按菜单 (`bookshelf_style1_page.dart`)

- [ ] 置顶
- [ ] 移动分组
- [ ] 详情（跳转 BookInfo）
- [ ] "整理"入口（`activity_arrange_book.xml`）
...
```
Task UI-1:  阅读器底栏 + 顶栏自动隐藏 + 更多菜单
Task UI-2:  阅读器设置补全（字体/TTS/自动阅读/点击行为/翻页键）
...
> 最后更新：2026-07-12 | 引擎 v0.5.6 | Focus: UI 复刻（S1 阅读器优先，RSS 延后）| 参考：[语雀 Wiki](https://www.yuque.com/legado/wiki)
