...
## 2026-08-01：当前 Android 设备基线纠正

- 当前设备：雷电模拟器 `emulator-5558`，Android 9 / API 28，`720x1280`，DPI `320`。
- ADB：`D:\Android\platform-tools\adb.exe`；原版包为 `io.legado.app.releaseS`，版本 `3.26.071309`。
- 当前设备只安装原版 Beta，重构版尚未安装，因此本次没有重构版实机截图，也不宣称 UI 像素验收通过。
- 当前原版只读参考集：`docs/ui-reference/legacy-beta/emulator-5558/`，共 65 张截图；截图中的棕色/红色主题属于设备当前主题配置，不是固定色值。
- 旧的 `emulator-5556` 记录保留为历史采集记录，不作为当前设备门禁；后续对照统一使用 `emulator-5558`，并固定同数据、同主题、同启动状态。
