...
| 统一 AppError | `search/explore/get_book_info/get_toc/get_content` 已使用 Rust `AppError`；其它 FFI 公开 API 仍大量 `Result<T, String>` | 部分完成 | 继续迁移校验、调试、数据库边界 |
...
本报告只固化差距和执行顺序；CoreApi 首批、Riverpod 样板、CI 门禁和书源主要入口 `AppError` 已完成，下一批继续迁移校验/调试/数据库错误边界，不改变 Provider 或正文行为。
