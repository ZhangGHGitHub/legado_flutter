...
| 统一 AppError | `search/explore/get_book_info/get_toc/get_content/get_content_with_next_chapter/validate_source/debug_search/debug_toc` 及 23 个 `db_*` 入口已改为 Rust `AppError`；其它 FFI 公开 API 仍有 `Result<T, String>` | 部分完成 | 继续迁移其它公开 FFI 错误和 Dart 统一映射 |
...
本报告只固化差距和执行顺序；CoreApi 首批、Riverpod 样板、CI 门禁、书源主链和数据库入口 `AppError` 已完成，下一批继续迁移其它公开 FFI 错误，不改变 Provider 页面事实源或正文行为。
